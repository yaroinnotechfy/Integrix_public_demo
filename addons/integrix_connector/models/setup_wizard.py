...
class IntegrixSetupWizard(models.TransientModel):
    ...
    sync_direction = fields.Selection([("ix2odoo","Integri-x → Odoo"),
                                       ...
                                       ("two_way","Two-way (advanced)")], default="odoo2ix")
    ...
    def default_get(self, fields_list):
        ...
        # автозаповнення company_info JSON (рядком)
        ...
        # підтягнути існуючі налаштування
        ...
        if config:
            ...
            # Мапінг: або з конфіга, або дефолтний, якщо порожньо
    ...
    @api.onchange("step")
    def _onchange_step_fill_company_info(self):
        for w in self:
            if w.step == "0a" and not (w.signup_company_info or "").strip():
                ICP = self.env["ir.config_parameter"].sudo()
                db_uuid = ICP.get_param("database.uuid") or ""
                w.signup_company_info = json.dumps({"db_uuid": db_uuid, "company_id": int(self.env.company.id)})
    ...
    def action_signup_submit(self):
        ...
        # companyInfo має бути РЯДКОМ (JSON string)
        try:
    ...
    def action_test_connection_wizard(self):
        ...
        try:
            ...
            if r.status_code >= 400:
                self.ping_status = "AUTH FAIL %s" % r.status_code
            ...
            self.ping_status = "AUTH ERROR: %s" % e
        ...
        try:
            ...
            self.ping_status = "OK" if pr.status_code == 200 else "HTTP %s" % pr.status_code
            try:
