...
class IntegrixPush(models.AbstractModel):
    ...
    def _cfg(self):
        ...
        if not cfg or not (cfg.api_email and cfg.api_password):
            raise UserError(_("Configure Integri-x Settings first (API Email/Password)."))
        if not (cfg.base_url or "").strip():
            cfg.sudo().write({"base_url": "https://app-winnerei-dev-1taskapp-api.azurewebsites.net/"})
        return cfg
    ...
    def _asset_payload(self, rec):
        ...
        return {
            "name": rec.name or f"Odoo {rec.id}",
            "externalId": rec.x_integrix_external_id,
            "parentId": parent_ext,
        }
    ...
    def push_equipment(self, records):
        ...
        ok, remote = self.env['integrix.client'].fetch_company_assets(
            cfg.base_url, cfg.api_email, cfg.api_password, cfg.company_id,
        ...
        if new_assets:
            ...
            self.env["ir.config_parameter"].sudo().set_param(
                "integrix_connector.last_sync_dt", fields.Datetime.now()
            )
            return {"pushed": len(new_assets), "errors": 0,
                    "skipped": skipped_unlinked + skipped_duplicates, "details": result}
        else:
            return {"pushed": 0, "errors": 0,
                    "skipped": skipped_unlinked + skipped_duplicates, "details": []}
